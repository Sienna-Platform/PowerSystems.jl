...
function from_openapi(po::PO.HydroPumpTurbine, refs::OpenAPIRefs, ::ComponentBaseUnit)
    return HydroPumpTurbine(;
        ...
        operating_mode = (if po.operating_mode isa Union{Nothing, IC.Absent}; HydroPumpTurbineStatus.OFF; else; HydroPumpTurbineStatus(po.operating_mode.value); end),
        ...
        active_power_pump = (if po.active_power_pump isa Union{Nothing, IC.Absent}; 0.0; else; po.active_power_pump; end),
        ...
        prime_mover_type = (if po.prime_mover_type isa Union{Nothing, IC.Absent}; PrimeMovers.PS; else; PrimeMovers(po.prime_mover_type.value); end),
    )
end
...
function from_openapi(po::PO.HydroPumpTurbine, refs::OpenAPIRefs, ::NaturalUnit)
    return HydroPumpTurbine(;
        ...
        time_at_status = (if po.time_at_status isa Union{Nothing, IC.Absent}; INFINITE_TIME; else; po.time_at_status; end),
        operation_cost = convert_cost(po.operation_cost.value)::OperationalCost,
        active_power_pump = (if po.active_power_pump isa Union{Nothing, IC.Absent}; 0.0; else; po.active_power_pump / po.base_power; end),
        efficiency = _turbinepump_from_po(po.efficiency),
        transition_time = _turbinepump_from_po(po.transition_time),
        minimum_time = _turbinepump_from_po(po.minimum_time),
        travel_time = (if po.travel_time isa Union{Nothing, IC.Absent}; nothing; else; po.travel_time; end),
