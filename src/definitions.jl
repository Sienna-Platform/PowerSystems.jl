...
function OperationalStates.T(value::Bool)
    return throw(
        ArgumentError(
...
Base.convert(::Type{OperationalStates.T}, value::Bool) = OperationalStates.T(value)
...
function CommitmentModes.T(value::Bool)
    return throw(
        ArgumentError(
...
Base.convert(::Type{CommitmentModes.T}, value::Bool) = CommitmentModes.T(value)
...
The on/off lifecycle is separate, in `status::OperationalStates.T`.
