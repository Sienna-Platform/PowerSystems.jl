...
function from_openapi(po::PO.EmissionsData, ::OpenAPIRefs)
    return EmissionsData(;
        name = po.name,
        pollutant = PollutantType.T(po.pollutant.value),
        emission_rate = convert_cost(po.emission_rate),
        basis = EmissionBasis.T(po.basis.value),
        start_up_adder = _or_default(po.start_up_adder, 0.0),
        mass_unit = MassUnit.T(po.mass_unit.value),
        energy_unit = EnergyUnit.T(po.energy_unit.value),
...
function from_openapi(po::PO.CombinedCycleBlock, ::OpenAPIRefs)
    return CombinedCycleBlock(;
        name = po.name,
        configuration = CombinedCycleConfiguration.T(po.configuration.value),
...
from_openapi(po::PO.ImpedanceCorrectionData, ::OpenAPIRefs) =
    ImpedanceCorrectionData(;
        ...
        transformer_winding = WindingCategory.T(po.transformer_winding.value),
